...
MUTATIONS = [
 ...
 # Re-anchored in round 8: the commit is now Perform(TrayGuardedOperation), so the readable permission
 # comes back beside THAT. The property is untouched -- no permission may be legible on this surface --
 # and it is still the return-type sweep that kills it, not a search for the retired name.
